...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, dataset_name: str, run_id: int = None, inference_mode= False,display_name: str = None,
                 result_only=False):
        ...
        if os.path.isfile(tracker_module_abspath):
            if inference_mode:
                tracker_module = importlib.import_module('lib.test.tracker.{}'.format(self.name + '_inf'))
            else:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            is_last = frame_num == len(seq.frames) - 1 # check the last frame of the video
            out, spike_rate_dict, spike_rate_dict_temp = tracker.track(image, info)
...
def calculate_average_spike_rates(spike_rate):
    """
    calculate the average sfr of each layer at each time step (across all images).
    return format: {layer_name: [avg_rate_t0, avg_rate_t1, ...]}
    """
    average_rates = {}

    for layer_name, image_data in spike_rate.items():

        all_rates = []
        for image_key, rates in image_data.items():
            all_rates.append(rates)

        import numpy as np
        all_rates = np.array(all_rates)  # shape: (num_images, time_steps)

        avg_rates = np.mean(all_rates, axis=0)
        average_rates[layer_name] = avg_rates.tolist()
...
def save_average_spike_rates(average_rates, video_name, save_dir="spike_rate_data"):
    ...
    print(f"avg sfr file save : {file_path}")
    return file_path


def load_average_spike_rates(video_name, save_dir="spike_rate_data"):
    """
    load average spike rates from json
    ...
    Returns:
        average_rates: dict of avg sfr
        metadata: dict include timestamp, save_time
    """
    ...
    try:
        ...
        print(f"successful loading sfr: {file_path}")
        return average_rates, metadata

    except json.JSONDecodeError as e:
        print(f"JSON format wrong: {e}")
        return None, None
    except Exception as e:
        print(f"loading wrong: {e}")
        return None, None
