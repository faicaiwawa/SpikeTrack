...
def build_dataloaders(cfg, settings):
    ...
    # data_processing_val = processing.SeqTrackProcessing(search_area_factor=search_area_factor,
    #                                                       output_sz=output_sz,
    #                                                       center_jitter_factor=settings.center_jitter_factor,
    #                                                       scale_jitter_factor=settings.scale_jitter_factor,
    #                                                       mode='sequence',
...
def get_optimizer_scheduler(net, cfg):
    ...
    for n, p in net.named_parameters():
        ...
        if cfg.DATA.TEMPLATE.NUMBER > 1:
            if "mrm" in n or "decay" in n or "pos_embed" in n :
                params_base_lr.append(p)
            else:
                params_multiplied_lr.append(p)
        else:         
            if "mrm" in n or "decay" in n or "pos_embed" in n or "decoder" in n:
                params_base_lr.append(p)
            else:          
                params_multiplied_lr.append(p)
    ...
    if is_main_process():
        if cfg.DATA.TEMPLATE.NUMBER > 1:
            print(" lr*0.1 for training head ") 
        else :
            print(" ori lr for training head ")   
