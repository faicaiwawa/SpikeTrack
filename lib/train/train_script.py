...
from lib.train.actors import SpikeTrackActor
# for import modules
import importlib
from lib.utils.focal_loss import FocalLoss

def run(settings):
    settings.description = 'Training script for SpikeTrack'
    ...
    # Create network
    if settings.script_name == "spiketrack":
        net = build_spiketrack(cfg)
    else:
    ...
    if settings.script_name == "spiketrack":
        ...
        actor = SpikeTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
